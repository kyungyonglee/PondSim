...
//    is_max = false;
  ...
  //  if(nodei->getAddress(cache) > max_addr){
   //   max_addr = nodei->getAddress(cache);
//      is_max = true;
//    }
...
//    while (shortcut_address == nodei->getAddress(cache) ) {
    ...
    int desired_shortcut = log(net_size);
    while (shortcut_count < desired_shortcut) {
 ...
 //         if(is_max == true){
 //           cout << "short cut edge from " << nodei->getAddress( cache) << " to " << nodej->getAddress(cache) <<endl;
//            cout << nodei->getAddress(cache)  << " number of shortcut connection is " << shortcut_count << endl;
 //         }
...
void DeetooNetwork::createEvenNet(int net_size) {
  ...
  while(node_map.size() < 2) 
  {
    my_int r_addr = (my_int)(_r_short.getDouble01() * (WMAX) );
    if (node_map.find(r_addr) == node_map.end() && r_addr != 0){
      AddressedNode* anode = new AddressedNode(r_addr, items, n_index);
      n_index++;
//      cout << "first address: " << r_addr << endl;
  ...
  while( node_map.size() < net_size) {
    ...
    my_int new_addr;
    if (dist1 >= dist2) {
      new_node = new AddressedNode(r_addr1,items, n_index);
      new_addr = r_addr1;
    }
    else {
      new_node = new AddressedNode(r_addr2,items, n_index);
      new_addr = r_addr2;
    }
    n_index++;
