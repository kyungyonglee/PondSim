...
int main(int argc, char *argv[]) 
{
  //int max_node = 100000;
  if (argc < 7) {
    cerr << "Usage: " << argv[0] << ", number of nodes, node_fail_prob, p_drop_prob, fail_fail_drop_prob, allowed timeout, wait_factor(true or false)" << endl;
  }
  int run_index = 0;
  ...
  int allowed_time = atoi(argv[5]);
  bool is_wait_factor = false;
  string wait_factor_mode = argv[6];
  if(wait_factor_mode.compare("true") == 0){
    is_wait_factor = true;
  }
...
//  Ran1Random ran_no;
  ...
  map<int,map<int,int> > attr_val_count;
  long total_latency = 0;
  long total_run_count = 0;
  while ( ni->moveNext() ) {
    if(total_run_count > 10000){
      break;
    }
    ...
    total_run_count++;
    total_latency += cached_net->Latency ;
...
//    cout << "depth = =" << c_in_depth << endl;
  ...
  cout << nodes << "\t" <<(total_latency/total_run_count) << endl;
}    
